...
class ResponsiveParser{
  ...
  ResponsiveParser(this.allowedElements);
  ...
  ResponsiveStylesheet getStylesheet(dom.Element element){
    ...
    if(tag != null && defaultStylesheet != null && defaultStylesheet.containsKey(tag)){
      findedStylesheet = defaultStylesheet[tag];
    }

    if(tag != null && stylesheet != null && stylesheet.containsKey(tag)){
      findedStylesheet = findedStylesheet == null ? stylesheet[tag] :
          ResponsiveStylesheet.cascade(findedStylesheet, stylesheet[tag], mergeBoxProperties: true);
    }
  ...
  RichText parseHTML(String html, bool renderNewLines, Map<String, ResponsiveStylesheet> customStylesheet){
  ...
  InlineSpan parseDomElement( dom.Element node, ResponsiveStylesheet lastStyle ){
    ...
    if(allowedElements.isNotEmpty && node.localName != 'body' && !allowedElements.contains(node.localName)){ return null; }
    ...
    ResponsiveStylesheet localStylesheet = getStylesheet(node),
        ...
        childStylesheet = localStylesheet == null ? lastStyle : ResponsiveStylesheet.cascade(lastStyle, localStylesheet);
    ...
    if(localStylesheet == null){
      ...
      localStylesheet = ResponsiveStylesheet.cascade(lastStyle, localStylesheet, mergeBoxProperties: true);
    }
    ...
    switch(node.localName){
      ...
      case "p":
        myChildren = [
          TextSpan( text: '\t' * (lastStyle?.textIndent ?? 0) )
        ]..addAll(myChildren);
        break;
    }
  ...
  @visibleForTesting
  ResponsiveStylesheet applyHtmlAttributes(dom.Element element, ResponsiveStylesheet lastStyle){

    ResponsiveStylesheet stylesheet = ResponsiveStylesheet().merge(lastStyle, mergeBoxProperties: true);
      ...
      switch(attribute){
        ...
        case 'align':
          switch(element.attributes['align']){
            case 'right':   stylesheet.textAlign = TextAlign.right; break;
            case 'left':    stylesheet.textAlign = TextAlign.left; break;
            case 'center':  stylesheet.textAlign = TextAlign.center; break;
            case 'justify': stylesheet.textAlign = TextAlign.justify; break;
          }
          break;
  ...
  @visibleForTesting
  InlineSpan getSpanElement(dom.Element element, List<InlineSpan> children, ResponsiveStylesheet lastStyle){
    ...
    ResponsiveStylesheet localStylesheet = applyHtmlAttributes(element, lastStyle);
    ...
    if(lastStyle != null && span != null){
      ...
      WidgetSpan blockSpan = WidgetSpan(
        ...
        child: Container(
          width:
          localStylesheet.width ?? (
              localStylesheet.displayStyle == DisplayStyle.block ? double.infinity : null
          ),
